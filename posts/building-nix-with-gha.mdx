...
nature and features it brought. I needed a tool that simplified the management of servers
but solutions like Ansible were too involved and finicky for my use-case. At the time,
...
After some research, I discovered NixOS. We gave it a try and in a few weeks had switched
...
the single sources of truth for our systems, and we did not have to worry about forgotten
...
Nowadays, I use GitHub Actions for a variety of jobs. For example, this website and the
APIs that support it are supported by GitHub Actions. Each code repository has a Dockerfile
and GitHub workflow file that builds and an OCI image and pushes it to a registry.
...
my old hobbies. The past three months I have dived back into the world of NixOS and
...
that are cached by [Cachix](https://cachix.org), especially for building my Raspberry
...
do work faster instead of waiting on things to compile or nix to finish evaluating.
